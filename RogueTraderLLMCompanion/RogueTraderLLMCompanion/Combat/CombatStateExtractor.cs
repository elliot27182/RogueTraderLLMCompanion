...
namespace RogueTraderLLMCompanion.Combat
{
    ...
    public class CombatStateExtractor
    {
        ...
        {
            // More robust psyker detection:
            // 1. Check if unit has any abilities with IsPsykerAbility flag
            // 2. Fallback to class name check for edge cases
            ...
            if (abilities != null)
            {
                foreach (var ability in abilities)
                {
                    ...
                    if (blueprint?.IsPsykerAbility == true)
                        return true;
                }
            }
            
            // Fallback: Check class progression (less reliable but catches edge cases)
        ...
        private List<AbilityInfo> ExtractAbilities(BaseUnitEntity unit)
        {
            ...
            foreach (var ability in unitAbilities)
            {
                ...
                abilities.Add(new AbilityInfo
                {
                    ...
                    APCost = GetAbilityAPCost(ability),
        ...
        private int GetAbilityAPCost(Ability ability)
        {
            // VERIFIED API: AbilityData.CalculateActionPointCost() (line 2433 of AbilityData.cs)
            // This uses the runtime Rulebook to account for buffs, debuffs, and other modifiers
            // Much better than the static BlueprintAbility.ActionPointCost
            try
            {
            ...
            {
                // Fallback to base cost if runtime calculation fails
                // BlueprintAbility does not have a direct ActionPointCost property
                Main.LogWarning($"Failed to calculate AP cost for {ability?.Name}: {ex.Message}");
                return 1;
            }
        }
